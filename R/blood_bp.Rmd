...
ul <- kfas.linear.att[,1] + 1.94*sqrt(V_mu)
ll <- kfas.linear.att[,1] - 1.94*sqrt(V_mu)
```
...
##lines(kfas.optim$muhat,lty=2)
legend("topright",legend=c("Systolic","filtered"),lty=c(NA,2),pch=c(19,NA))
```
...
legend("topright",legend=c("Systolic","smoothed state"),lty=c(NA,2),pch=c(19,NA))
```
