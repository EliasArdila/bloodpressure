...
```{r }
kfas.local.linear.a <- coredata(kfas.linear.a)  

k.a <- apply(kfas.local.linear.a,1,sum)
...
legend("topright",legend=c("Systolic","filtered"),lty=c(1,2))
```
...
plot(syst,pch=19,type="p",xlab="",ylab="")
lines(alphahat[,1],lty=2)
legend("topright",legend=c("Systolic","smoothed state"),lty=c(1,2))
```
...
funModel <- function(parm){
   m1 <- dlmModPoly(dV=parm[1],dW=parm[2:3])
   return(m1)
}

oMLE.DLM <- dlmMLE(syst, parm=theta,build=funModel)
...
```{r,fig.align='center' }
plot(ff,type="l",xlim=c(0,tdx[f_f]))
