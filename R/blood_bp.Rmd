...
ul <- kfas.linear.att[,1] + qnorm(1-0.025)*sqrt(V_mu)
ll <- kfas.linear.att[,1] + qnorm(0.025)*sqrt(V_mu)
```
...
lines(coredata(syst.pred) ~ xxt,type="o",col="red",pch=19)
legend("bottomright",legend=c("Systolic","Smoothed","Predicted"),lty=c(1,2,1),pch=c(19,NA,19),
col=c("darkgrey","black","red"),cex=.9,bty="n")
```
...
ul <- kfas.linear.att.diast[,1] + qnorm(1-0.025)*sqrt(V_mu.diast)
ll <- kfas.linear.att.diast[,1] + qnorm(0.025)*sqrt(V_mu.diast)
```
...
lines(alphahat[,1] ~ xt,lty=2)
lines(coredata(diast.pred) ~ xxt,type="o",col="red",pch=19)
legend("bottomright",legend=c("Diastolic","Smoothed","Predicted"),lty=c(1,2,1),pch=c(19,NA,19),
col=c("darkgrey","black","red"),cex=.9,bty="n")
```
